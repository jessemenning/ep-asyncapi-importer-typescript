...
import { OnCancel } from '../../src/_generated/@solace-iot-team/sep-openapi-node/core/CancelablePromise';
...
function parseCookies(response: Response) {
  ...
  // console.log(`\n\n\nOpenApi.parseCookies(): raw=${JSON.stringify(raw, null, 2)}`);
 ...
 * @throws ApiError
 */
export const customRequest = <T>(config: OpenAPIConfig, options: ApiRequestOptions): CancelablePromise<T> => {
    return new CancelablePromise(async (resolve, reject, onCancel) => {
        try {
            ...
            if (!onCancel.isCancelled) {
                ...
                // console.log(`\n\n\n ------- BEGIN customRequest:`);
                // console.log(`customRequest(): response.headers=${JSON.stringify(response.headers.raw(), null, 2)}`);
